...
bool pgn_next_game(pgn_t * pgn) {
   ...
   while (true) {
	  ...
	  } else if (my_string_equal(name,"Score")) {
         strcpy(pgn->score,value);  // printf("score=%s\n",pgn->score);		// @ed
	
	  } else if (my_string_equal(name,"Depth")) {
         strcpy(pgn->depth,value);  // printf("depth=%s\n",pgn->depth);		// @ed

   	  } else if (my_string_equal(name,"Elo")) {
         strcpy(pgn->learn,value);  // printf("elo=%s\n",pgn->elo);			// @ed
 	  }		 	
