   ...
   while (pgn_next_game(pgn)) {
      ...
      while (pgn_next_move(pgn,string,sizeof(string))) {
         ...
         if (ply < MaxPly) {
			...
			Book->entry[pos].score = atoi(pgn->score);		// @ed
			Book->entry[pos].depth = atoi(pgn->depth);		// @ed
			Book->entry[pos].learn = atoi(pgn->learn);		// @ed
...
//    write_integer(file,2,0);								// @ed
//    write_integer(file,2,0);   							// @2d
      write_integer(file,1,Book->entry[pos].score&255);		// @ed
      write_integer(file,1,Book->entry[pos].score>>8);		// @ed
      write_integer(file,1,Book->entry[pos].depth);			// @ed
      write_integer(file,1,Book->entry[pos].learn);			// @ed
   }
